...
The workflow is quite simple: write something -> commit, upload to GiHub -> Netlify builds and deploys. This post is about when that _'write something'_ part is about R and written in [RMarkdown](https://rmarkdown.rstudio.com/). Let's go! 🦔
...
So it allows us to get all the MDX files and use them as data -- we extract the title and date and an extract and that's what goes in the blog listing place. Pretty cool, eh?
