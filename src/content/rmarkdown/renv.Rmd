...
It's pretty easy to see package dependencies with renv (not as easy as showpackage `r emo::ji("stuck_out_tongue_winking_eye")`). We can see what dependencies come from our .R files, for example, which is a nice feature:  
```{r eval = FALSE}
library(dplyr)
library(stringr)
...
All in all, renv is a nice, well-designed package that is easy to use. I've already used starting using it, and once it gets to CRAN, I'll be using it 'in production'. Nice job, RStudio `r emo::ji("+1")`
